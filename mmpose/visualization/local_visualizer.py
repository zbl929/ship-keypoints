...
class PoseLocalVisualizer(OpencvBackendVisualizer):
    ...
    实现：
     显示track——id
    """
    def _draw_instances_bbox_and_trackid(self, image: np.ndarray,
                             instances:  PoseDataSample) -> np.ndarray:
        """Draw bounding boxes and corresponding labels of GT or prediction.
        ...
        if 'bboxes' in instances.pred_instances:
            ...
            bboxes = instances.pred_instances.bboxes
            self.draw_bboxes(
                bboxes,
                edge_colors=self.bbox_color,
                alpha=self.alpha,
        ...
        if 'track_id' in instances:
            ...
            for i, (pos, label) in enumerate(zip(positions, track_id)):
                id_text = f'track_id: {track_id[i]}'
                if isinstance(self.bbox_color,
                              tuple) and max(self.bbox_color) > 1:
                    facecolor = [c / 255.0 for c in self.bbox_color]
                else:
        ...
        if draw_pred:
            ...
            if 'pred_instances' in data_sample:
                pred_img_data = self._draw_instances_kpts(
                    ...
                    show_kpt_idx, skeleton_style)
                if draw_bbox:
                    if 'track_id' in data_sample:
                        pred_img_data = self._draw_instances_bbox_and_trackid(
                            pred_img_data, data_sample)
